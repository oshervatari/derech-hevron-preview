...
DH.residents = [
  {
    id: 'r-001', accessCode: 'demo-101', plotNo: 1,
  ...
  {
    id: 'r-002', accessCode: 'demo-102', plotNo: 2,
  ...
  {
    id: 'r-003', accessCode: 'demo-103', plotNo: 3,
  ...
  {
    id: 'r-004', accessCode: 'demo-104', plotNo: 4,
  ...
  {
    id: 'r-005', accessCode: 'demo-105', plotNo: 5,
...
/* ---- Demo filler: plots 6-40 ------------------------------------------------
 * The public queue shows plot numbers only (no names), so the filler records
 * carry just what the queue needs: plotNo, signingDate, lotteryNo.
 * Signing dates are deterministic demo values; lotteryNo counts up within each
 * shared date. Replace the whole list with real data behind a backend later. */
(function () {
  var dates = [
    '2023-11-02', '2023-11-19', '2023-12-01', '2023-12-15', '2024-01-09',
    '2024-01-24', '2024-02-06', '2024-02-20', '2024-03-05', '2024-03-18',
    '2024-04-02', '2024-04-16', '2024-05-01', '2024-05-15', '2024-06-03',
  ];
  var perDate = {};
  DH.residents.forEach(function (r) {
    perDate[r.signingDate] = Math.max(perDate[r.signingDate] || 0, r.lotteryNo || 1);
  });
  for (var i = 6; i <= 40; i++) {
    var d = dates[(i * 7) % dates.length];
    perDate[d] = (perDate[d] || 0) + 1;
    DH.residents.push({
      id: 'r-' + ('000' + i).slice(-3), accessCode: null, plotNo: i,
      name: 'בעל חלקה ' + i, originalAptNo: String(i), signingDate: d, lotteryNo: perDate[d],
      entitlementSqm: [75, 81, 104][i % 3], entitlementRooms: [3, 3, 4][i % 3],
      parking: { included: true, count: 1 }, storage: { included: true, minSqm: 5 },
      selected: null,
      adminNotes: '',
    });
